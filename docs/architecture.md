...
- `WhisperKitTranscriber` feeds audio into WhisperKit's live transcriber and
  ...
  without the prompt and uses the winning attempt. A hold that carried no voice
  energy at all finishes empty without a final decode, so Whisper never gets the
  chance to hallucinate into silence. The model remains resident
  between dictations.
