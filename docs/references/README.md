...
| [asr-engine-selection.md](asr-engine-selection.md) | ASR engine research & decision record: WhisperKit turbo as the incumbent runtime baseline; full large-v3 as the acceptance-gated candidate; on-device acceptance protocol | Multi-agent research over the asr-* references, adversarially verified | Decision record behind the runtime ASR choice |
