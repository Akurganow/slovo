...
/// returns the captured audio as a §18.3 `SlovoCore.AudioBuffer`.
public final class AVAudioEngineRecorder: AudioRecorder, @unchecked Sendable {
    ...
    public func stop() async throws -> SlovoCore.AudioBuffer {
        engine.inputNode.removeTap(onBus: 0)
        engine.stop()

        let samples = lock.withLock { accumulatedSamples }
        return SlovoCore.AudioBuffer(samples: samples, format: targetFormat)
    }
