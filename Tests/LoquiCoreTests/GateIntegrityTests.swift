...
//
// The probe is ARMED only when `LOQUI_GATE_SELFTEST=red`; default `swift test`
// leaves it disarmed (GREEN). The DEMONSTRATION is the evidence:
//   - `LOQUI_GATE_SELFTEST=red swift test` exits NON-ZERO and names this test;
...
struct GateIntegrityTests {
    private static var isArmed: Bool {
        ProcessInfo.processInfo.environment["LOQUI_GATE_SELFTEST"] == "red"
    }
    ...
    func gateGoesRedWhenSelfTestArmed() {
        if Self.isArmed {
            Issue.record("LOQUI_GATE_SELFTEST=red: gate-integrity probe ARMED — gate must fail")
        } else {
            #expect(Self.isArmed == false, "disarmed by default; arm via LOQUI_GATE_SELFTEST=red")
        }
    }
}
