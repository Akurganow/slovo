...
struct AppShellPackagingTests {
    ...
    func appShellUsesAccessoryMenuBarShape() throws {
        let main = try Self.strippingComments(from: String(
            contentsOfFile: Self.packageRoot.appending(path: "Sources/loqui/main.swift").path,
            encoding: .utf8
        ))
    ...
    func appShellBuildsProductionCompositionAndRoutesHotkey() throws {
        let composition = try Self.strippingComments(from: Self.source("Sources/loqui/AppComposition.swift"))
        let delegate = try Self.strippingComments(from: Self.source("Sources/loqui/AppDelegate.swift"))
    ...
    func infoPlistDeclaresMenuBarAgentAndTccUsage() throws {
        ...
        #expect(plist["CFBundleIdentifier"] as? String == "com.loqui.app")
        #expect(plist["CFBundleExecutable"] as? String == "loqui")
    ...
    func entitlementsAllowAudioInputWithoutSandbox() throws {
        let data = try Data(contentsOf: Self.packageRoot.appending(path: "loqui.entitlements"))
        let plist = try #require(PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
    ...
    func signingScriptUsesHardenedRuntimeAndOptionalNotarization() throws {
        ...
        #expect(source.contains("loqui.entitlements"))
