...
// `Sources/LoquiCore/Contracts/`; the symbols are CURRENTLY supplied by the
// `_RedScaffold_Contracts.swift` stub so this target compiles — the implementer
// deletes the scaffold and these tests bind to the real `import LoquiCore`).
//
...
struct ContractsTests {
    ...
    func valueTypesHaveExactSpecShapes() {
        ...
        let noExpansion = Term(term: "loqui", expansion: nil, lang: .auto, weight: 1)
