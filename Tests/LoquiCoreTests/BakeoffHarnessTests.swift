...
struct BakeoffHarnessTests {
    ...
    private static func clip(index: Int) -> LoquiCore.AudioBuffer {
        let format = AVAudioFormat(standardFormatWithSampleRate: 16_000, channels: 1)!
        return LoquiCore.AudioBuffer(samples: [Float(index)], format: format)
    }

    /// Three code-switched transcripts (Cyrillic + PR + GitHub), one per clip.
    private static func codeSwitchedScript(_ audio: LoquiCore.AudioBuffer) -> Result<String, TranscriptionError> {
        .success("запушь PR в GitHub репозиторий")
    }

    /// Three collapsed transcripts (Latin anchors transliterated away).
    private static func collapsedScript(_ audio: LoquiCore.AudioBuffer) -> Result<String, TranscriptionError> {
